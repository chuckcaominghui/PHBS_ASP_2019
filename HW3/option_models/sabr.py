...
class ModelNormalMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        forward = spot * np.exp(texp*(self.intr - self.divr))
        trange = 0.01
        step = int(texp/trange)
        
        vol = np.ones((self.sample, step+1))
        np.random.seed(12345)
        Z1 = np.random.normal(size=(self.sample, step))
        
        vol[:,1:] = np.cumprod(np.exp(self.alpha*np.sqrt(trange)*Z1 - 0.5*self.alpha**2*trange), axis = 1)
        vol = sigma * vol[:,:-1]

        np.random.seed()
        W1 = self.rho * Z1 + np.sqrt(1 - self.rho**2) * np.random.normal(size = (self.sample, step))
        pricemock = forward + np.cumsum(vol*W1*np.sqrt(trange), axis = 1)
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        weight = np.ones(self.step + 1)
        weight[1::2] = 4
        weight[2::2] = 2
        weight[-1] = 1
        weight = np.resize(weight, (self.sample, self.step + 1))

        keyv = trange/3 * np.cumsum(weight*vol**2, axis=1)

        Cond_MC_spot = spot * np.exp(self.rho/self.alpha*(vol[:,-1]-vol[:,0])-self.rho**2/2*keyv[:,-1])
        Cond_MC_vol = np.sqrt((1-self.rho**2)*keyv[:,-1]/texp)
        Cond_MC_price = self.bsm_model.price(strike, Cond_MC_spot[:,None], texp, Cond_MC_vol[:,None], cp_sign=cp_sign)
        
        price = np.mean(Cond_MC_price, axis = 0)
...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        keyv = trange/2 * np.cumsum(weight*vol**2, axis=1)

        Cond_MC_spot = spot + self.rho/self.alpha*(vol[:,-1]-vol[:,0])
        Cond_MC_vol = np.sqrt((1-self.rho**2)*keyv[:,-1]/texp)
        Cond_MC_price = self.normal_model.price(strike, Cond_MC_spot[:,None], texp, Cond_MC_vol[:,None], cp_sign=cp_sign)
